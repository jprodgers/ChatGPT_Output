...
- **Export**
  - Filename pattern field and one-click **Export PNG** button. Use `#` characters for an auto-incremented counter (default `user://screenshot####.png` saves to `user://screenshot0000.png`, then `0001`, etc.). Desktop exports save without clearing the grid; in web builds, exports trigger a browser download instead of writing to the sandboxed filesystem.
- **Wolfram**
