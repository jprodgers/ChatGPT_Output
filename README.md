...
- A GDExtension implementation for the heavy steppers (Wolfram sweep, ants, turmites, totalistic rules like GoL/Day & Night/Seeds, and the falling sand pile) lives in [`cpp/`](cpp/README.md); when built and present in `bin/`, the project will automatically use the C++ paths with a GDScript fallback if the library is missing. See [`docs/native_extension_troubleshooting.md`](docs/native_extension_troubleshooting.md) for help wiring up `godot-cpp` and verifying the binary is in the right place.
