...
3) **Point to an existing checkout elsewhere.** Set `GODOT_CPP_PATH` before running SCons (and `GODOT_HEADERS` if your headers live elsewhere, e.g., `.../godot-cpp/gdextension`):
...
- **Missing headers (`method_ptrcall.hpp`, `global_constants.hpp`).** You cloned `godot-cpp` but did not build it, or you are on a newer layout that puts almost everything under `gen/include`. Run SCons inside `godot-cpp` first with `generate_bindings=yes` so generated headers land in either `include/gen/...` (older layout) or `gen/include/...` (newer layout) and the `bin/` library exists. Also ensure the bundled headers exist inside `godot-cpp` (for example `godot-headers/`, `godot_headers/`, or `gdextension/`), or point `GODOT_HEADERS` to the folder that contains `gdextension_interface.h`.
