...
func step_wolfram(allow_wrap: bool = true) -> void:
	...
	if not _any_sim_busy():
		...
		if _enqueue_sim_task("wolfram", Callable(self, "_thread_compute_wolfram"), args):
			return
	if grid_size.y <= 0:
		return
...
func step_ants() -> void:
	...
	if not _any_sim_busy():
		...
		if _enqueue_sim_task("ants", Callable(self, "_thread_compute_ants"), args):
			return
...
func step_totalistic(birth: Array[int], survive: Array[int]) -> void:
	...
	if not _any_sim_busy():
		...
		if _enqueue_sim_task("totalistic", Callable(self, "_thread_compute_totalistic"), args):
			return
...
func step_turmites() -> void:
	...
	if not _any_sim_busy():
		...
		if _enqueue_sim_task("turmites", Callable(self, "_thread_compute_turmites"), args):
			return
	var remove_indices: Array[int] = []
...
func step_sand() -> void:
	...
	if not _any_sim_busy():
		...
		if _enqueue_sim_task("sand", Callable(self, "_thread_compute_sand"), args):
			return
